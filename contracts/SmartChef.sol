...
// added functionnalities by the faevswap's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when FAEV mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 faevReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake FAEV tokens to TheBushV1
    ...
    // Withdraw FAEV tokens from STAKING.
